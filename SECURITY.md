...
the account-owned approval row. The durable relay survives API-worker changes;
its lifetime and delivery semantics are defined in the
[API contract](apps/api/README.md). Only an authenticated agent for the same
account can receive or confirm it.
...
Repeating confirmation for that same fingerprint is idempotent under the API
contract, while a different, expired, wrong-account, or undelivered candidate
fails closed.
