...
5. Retain the explicitly authorized fact-backed combobox dispatcher to open a control whose displayed value contradicts a supplied fact. Attempt it once per observed state and yield to the model after a stale, occluded, or offscreen click, even when the recovery snapshot changes. Visible matching options go to the model: the snapshot does not establish custom option ownership, so the dispatcher neither clicks a page-wide match nor marks a field filled on that basis. Native selects retain their existing model decision path.
...
Regression coverage executes the browser loop with an offscreen Country combobox, a covered Country combobox, and a combobox whose click changes nothing. The recovery cases require the model to scroll or dismiss the obstruction before automatic opening and model-directed selection can finish. Shipping/Billing fixtures verify that Billing's visible Canada option reaches the model without an automatic click or a false Shipping completion. Model answers are deterministic fixtures; these tests establish dispatch and recovery behavior, not live model accuracy.
