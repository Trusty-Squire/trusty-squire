...
| Browser egress | Exercise arbitrary third-party fetch/XHR, SDK loading, and 3DS traffic using `browser-unrestricted-egress.test.ts` and `browser-decoupled-3ds.test.ts`; qualify the [egress contract](operator-tool-surface.md#browser-egress-is-unrestricted). |
