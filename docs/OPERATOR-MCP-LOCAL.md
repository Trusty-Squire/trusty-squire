# Operator MCP local staging runner

The operator Codex MCP runs the local `origin/staging` build through
...
On every launch, the wrapper fetches `origin/staging`, resets its dedicated
...
normal launches skip installation and builds. A changed staging commit or lockfile
...
rebuild of the current staging revision, run:
