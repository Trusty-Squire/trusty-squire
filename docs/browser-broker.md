...
- The profile-local dispatch journal fsyncs mutation entry and completion without
  ...
  replayed. Reconciliation keeps a
  confirmed payment submission as `done`, distinct from 3-D Secure-required and
  unknown outcomes.
- The journal is a record, not a gate: it never refuses browser replacement,
  blocks fresh starts, or wedges startup, regardless of unsettled entries.
  Recovery is always client-initiated (`recover` + the recover metadata); a
  session with uncertain outcomes stays recoverable, and new work is admitted
  unconditionally. The forwarder never fenced a
  fresh start on an in-flight call and adds no gate of its own.
...
- A code-proven pre-dispatch stale-ref failure on any mutating command
  ...
  `error: stale_ref`. These preconditions rely on the acceptance report's
  retained tool output as the failure evidence; without it, do not treat the
  reconciliation as confirmed.
