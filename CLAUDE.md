...
broker client (connect / open / command / close) reachable from any process, and
every instruction it issues — the acquire and `tab.page.goto` alike — carries a
deadline and is cancellable through the wire `abort` frame; `browserBusy` is a
strictly read-only probe of the one profile the broker serves. The refusal-code
mapping, the read-only contract, and the "a running tab family is not busy" rule
live in
...
| Env var                         | Default                   | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `API_ACCOUNT_HOURLY_LIMIT`      | `1000`                    | Per-account rolling-hour cap on the authed control plane (every `requireWeb`/`requireAgent`/`requireAny` route — vault store/list/use, grant mint, etc. — except `POST /v1/decide`, which a drive calls once per step and which is bounded by its own size checks instead; see [`docs/DESIGN-jev-platform-route.md`](docs/DESIGN-jev-platform-route.md)). DoS backstop; returns `429 {error:rate_limited, scope:account}`. The deployed-app egress PROXY runs on a separate grant-token path with an optional per-grant cap, so this doesn't throttle workloads. `<= 0` disables. In-memory / single-instance.                                                    |
