...
[`docs/PRODUCT.md`](docs/PRODUCT.md) owns product positioning and audiences. The
[README install section](README.md#install) owns the canonical supported-target
list, and [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) owns the system overview
and data flows. The MCP server runs on the user's machine; an API on Fly.io
handles persistence and orchestration.
...
- **Postgres persistence (Fly Postgres `trusty-squire-db`).** One database,
  ...
  `DecisionEvent`, `VaultAuditEvent`. (The second `trustysquire_inbox` database was dropped
...
| `TYPESAFE_API_KEY`                              | Platform TypeSafe key for `POST /v1/decide` (Jev). Set like the Telegram bot token: `flyctl secrets set TYPESAFE_API_KEY=…` on `trusty-squire-api`. Absent → the route answers `503 jev_unconfigured`. Users with their own vaulted `typesafe` credential keep using it (BYOK). See [`docs/DESIGN-jev-platform-route.md`](docs/DESIGN-jev-platform-route.md). |
...
Import `openTab`, `browserBusy`, and `BrowserBusy` from
`@trusty-squire/mcp/browser` (`apps/mcp/src/browser-busy.ts`). The broker-owned
status, cancellation, timing, and refusal contracts are defined in
...
|---|---|---|
| `TWOCAPTCHA_API_KEY` | — | Optional Tier 3 captcha solver — the escalation for a checkbox-family challenge the Tier 1/Tier 2 substrate handling doesn't clear. ~$0.003/solve. Spent on BOTH captcha paths: the provision captcha gate, and the general operate drive, where `observeSession` runs a best-effort auto-solve for a rendered hCaptcha / reCAPTCHA v2 / v3 challenge. On the operate path Turnstile is deliberately left to its own managed challenge, and spend is bounded per session — one attempt in flight at a time, a 30s cooldown after a failed one, a geometric backoff (30s base doubling to a 480s cap; resets on consumption) after a bought token expires unconsumed, and a hard stop on a page whose one-shot gate-style handoff has already been attempted and whose gate challenge renders again (`autosolve_disabled` audit; the challenge is left visible for the operator). **Preferred path is the VAULT, not this env var:** `connect`/`settings` advanced setup prompts for the key and stores it encrypted as the `2captcha` credential; both paths spend it through the injecting proxy (`use_credential`, `${SECRET}` in the `key` query / `clientKey` body) so the raw key never lives in the bot process or the MCP config. `buildTwoCaptchaSolver` (`apps/mcp/src/bot/captcha-solve.ts`, shared by both paths) prefers the vaulted cred and falls back to this env var for back-compat. The 2Captcha solve is a back-channel token fetch (the v2 token is injected into the user's real browser session and isn't IP-bound), so routing it through the vault adds no target-side fingerprinting. |
...
| Env var                         | Default                   | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `API_ACCOUNT_HOURLY_LIMIT`      | `1000`                    | Per-account rolling-hour cap on the authed control plane (every `requireWeb`/`requireAgent`/`requireAny` route — vault store/list, grant mint, etc. — except the server-side-use routes `POST /v1/decide` and `POST /v1/vault/use`, which neither consume this allowance nor fail when it is exhausted; authentication and route-specific checks still apply). DoS backstop; returns `429 {error:rate_limited, scope:account}`. The deployed-app egress PROXY runs on a separate grant-token path with an optional per-grant cap, so this doesn't throttle workloads. `<= 0` disables. In-memory / single-instance.                                                    |
