...
| `TRUSTY_SQUIRE_SERVER_IDLE_CHECK_INTERVAL_MS` | `300000` (5m) | Poll interval for the two idle bounds above. Kept well under the 20m no-session bound so it's actually enforced at ~20m rather than delayed toward ~50m by a coarser poll. |
