...
- `connect --json` prints exactly one typed report on stdout on every exit path (`state`, `sign_in_url`, `account`, `holder`, `browser_location`, plus a narrow `reason`) from the same value the human copy renders. `needs-sign-in` always carries its URL — the ceremony now waits no longer than the sign-in link lives, and a lapsed link reports `install_expired` instead. `browser_location` is reported by the path that placed the ceremony browser and survives into a failed run's report — a virtual display is named as such and carries no address, because the tunnel that reached it dies with the ceremony — `holder` is reported as read and a ceremony the profile gate refuses reports `busy` rather than a sign-in, `state` never says `connected` on a run that exits non-zero, a rejected flag answers `{"error":"usage"}` rather than a connection state, `--json` implies `--no-interactive`, and human output is unchanged.
