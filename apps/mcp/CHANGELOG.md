...
- Reconcile drive handles into canonical observations (#957)
- fix(mcp): resolve card refs against observed frame anchors (#956)
- feat(mcp): make drive handback refs actionable (#955)
- Bridge drive snapshot refs to compact-v2 allocator (#954)
- fix: omit unknown drive goal phase instead of done (#953)
- Allow extracted credentials across registrable domain (#952)
- Capture unique visible key values from page (#951)
- fix(mcp): copy keys from presentation layers (#950)
- refactor(mcp): leave drive choices to the model (#949)
- fix(mcp): find copied keys in modal layers (#948)
- fix(mcp): capture copied key from masked creation dialog (#947)
- Drive key goals through one-time key creation (#946)
- Drive goal values into creation forms and enforce target scope (#945)
- fix(mcp): capture all readable credentials before key-goal completion (#944)
- fix(mcp): capture covering shadow and frame overlays in drive (#943)
- Require secret-shaped evidence for drive key completion (#942)
- Fix drive key completion refusal loop (#940)
- fix(mcp): scope drive required-field guard to selected form (#941)
- Drive through required low-commitment onboarding choices (#939)
- fix(mcp): count only drive decisions and dispatched actions (#937)
- fix(mcp): dismiss model-observed covering layers before backtracking (#938)
- fix(mcp): follow OAuth popup after product navigation (#936)
- fix: submit emailed code through its form (#935)
- fix(mcp): redecide after redirected pre-act choice (#933)
- Fix drive email-code recognition and OAuth popup diagnostics (#934)
- fix(mcp): advance key creation after keys-page navigation (#932)
- Fix named-provider drive sign-in and OAuth handoff diagnostics (#931)
- fix(mcp): skip in-app fact filters and dismiss isolated overlays (#930)
- fix(mcp): recover a chosen drive control that left or was covered (#929)
- fix(mcp): press a checkbox anti-bot challenge from the operate drive (#928)
- fix(mcp): stop a revealed key's masked copy blocking drive completion (#927)
- fix(mcp): include the accessible label in the drive's pre-act row digest (#926)
- fix(mcp): never end a drive over a pre-act re-decide (#925)
- fix(mcp): complete a claimed enrollment without the browser Finish control (#924)
...
- A claimed enrollment now completes on the claim alone. The wizard's Finish control is a courtesy that closes the page early, not a completion gate: on a headless machine that page exists only behind the single-use pairing link, so gating on a control inside it could strand an enrollment the server had already established (a finished Google sign-in whose `session.json` held only the API base and a machine token). The claim is the fact the CLI polls; nothing re-adds a second signal or extends the deadline.
- The shared-display warning prints only where it is true: a real human-facing display, with the login tab in the browser the person is already using — the one later Trusty Squire sessions keep opening tabs in. The headless noVNC ceremony prints no warning at all (its tabs are the same owner's, for the same account, on a link handed to themselves); the noVNC banner still carries the URL and password.
- refactor(mcp): the broker is now the only path to a browser, and account identity is named by the call rather than the connection. The connect ceremony no longer falls back to a browser of its own, and `BOT_SELF_LAUNCH` is gone: the broker always performs the Turnstile-safe self-launch and hands every tab out from it. The wire `connect` carries no credential and the daemon starts with no enrollment, so the machine that most needs the shared browser — the one being enrolled — can reach it; `open`/`command`/`close` carry an optional account and the broker builds the API client for it per call, so a session that outlives a re-enrollment acts as the freshly enrolled account on its next account-acting call. Proven by `broker-only-path-acceptance.test.ts` against a real broker, real Chrome, and real MCP stdio servers: two concurrent sessions sharing one browser, and an enrollment completing while a session is live.
...
## 1.1.16-rc.4 (2026-09-21)

- `connect --json` writes newline-delimited typed reports on stdout — one complete report per line, whenever the run's answer changes, with the last line marked `terminal` — on every exit path (`state`, `sign_in_url`, `account`, `holder`, `browser_location`, plus a narrow `reason`) from the same value the human copy renders. the sign-in URL reaches the machine channel before the run starts waiting, `needs-sign-in` always carries its URL — the ceremony now waits no longer than the sign-in link lives, and a lapsed link reports `install_expired` instead. `browser_location` is reported by the path that placed the ceremony browser and survives into a failed run's report — a virtual display carries the noVNC address that reaches it, on a line written while that tunnel is up, and a ceremony that ends without any path reporting a placement is `unreachable` named with the failure — `holder` is reported as read — from Chrome's lock or the profile-operation lease, whichever holds it, and never the ceremony Chrome this run launched — and a ceremony refused by the profile gate or by a resident broker holding the browser reports `busy` rather than a sign-in (a broker that dies mid-ceremony reports `run_failed`, not a holder to wait for), `state` never says `connected` on a run that exits non-zero, a rejected flag answers `{"error":"usage"}` rather than a connection state, `account` carries the binding whenever a run proved one, with `account.providers` `null` when the provider probe could not read the profile rather than an empty list, `--json` implies `--no-interactive`, and human output is unchanged. (#913)
- fix(mcp): never vault a control identifier, and report a still-masked sibling key (#912)
- fix(mcp): key-goal completion reuses the capture flow, not a second policy (#911)
- fix(mcp): close the drive decider feedback loop (#908 follow-up) (#910)
- fix(mcp): put headed Chrome on the real screen instead of Xvfb (#909)
- fix(mcp): execute or drop decided drive acts, prefer dialog/overlay, bound OAuth (#908)
- fix(mcp): mint drive card-release approvals for the checkout page total (#899)
- fix(mcp): shape drive click candidates so docs and pickers never beat in-app work (#907)
- fix(mcp): try listed entries before repeating sibling tabs (#906)
- fix(mcp): settle in-page clicks and mark revealed keys without leaking them (#905)
- fix(mcp): keep signed-in app pages moving and surface submit field errors (#904)
- fix(mcp): decide before exploring and withhold INBOX until a submit (#903)
- fix(mcp): bind actions to snapshot identity and stop pre-existing verify waits (#902)
- fix(mcp): identify sections by destination and stop explorer loops (#901)
- fix(mcp): prefer destination nav over filters and try unvisited sections (#900)
- fix(mcp): pick inbox mail by opened To, not conversation-row date (#898)
- fix(mcp): resolve operate_drive oauth_login on links and stop oauth-only cycling (#897)
- fix(mcp): stop operate_drive from cycling reminted submit controls (#896)
- fix(mcp): finish operate_drive on the page's post-submit answer (#895)
- fix(mcp): fire reCAPTCHA callbacks in the page main world (#894)
- fix(mcp): resolve string-named reCAPTCHA callbacks on inject (#893)
- fix(mcp): classify rendered captcha and keep operate_drive solver polling (#892)
- fix(mcp): run captcha autosolve from operate_drive and stop adopting leftover tabs (#891)
- fix(mcp): poll inbox after submit even with leftover form controls (#889)
- fix(mcp): scope inbox reads to the session recipient and service host (#888)
- fix(mcp): fail closed to test:fast, not the full suite (#885)
- fix(mcp): offer INBOX when verification is off-page (#887)
- fix(ci): run mcp test:fast in the validation lap (#883)
- fix(mcp): prefer key-seeking controls after signup confirmation (#878)
- fix(mcp): click and type through Playwright actionability by default (#882)
- fix(mcp): treat heading change as progress and poll inbox before signup failure (#881)
- fix(mcp): rank needs-fill first and ask the inbox before widget-unready (#880)
- fix(mcp): commit listbox and combobox options without cached coordinates (#877)
- fix(mcp): split a disabled submit into in-flight, needs-fill, and widget-unready (#876)
- fix(mcp): rank operate_drive candidates by form, cover, failure, and page phase (#875)
- fix(mcp): keep offscreen checkout Pay controls reachable and refuse pay substitutes (#874)
- fix(mcp): stop operate_drive no-op loops and dishonest waits (#873)
- fix(mcp): gate Google session only on Google-dependent operations (#872)
- fix(mcp): carry operate_drive through checkout to card entry (#871)
- fix(mcp): shorten drive settling and defer broker startup observation (#870)
- fix(vault): reduce proxy latency by overlapping best-effort audit writes (#869)
- fix(mcp): improve drive picker handling and reduce snapshot noise (#868)
- fix(api): exempt server-side vault use from account rate limits (#867)
- fix(mcp): speed up drive startup by deferring to its first snapshot (#866)
- feat(mcp): add resumable goal-driven browser automation (#859)
...
- **BREAKING: every observation seal, screenshot redaction, and extraction
  ...
  field values, control labels, semantic headings, interleaved DOM text, and the
  live page URL (path and query included) verbatim. The standing captain
  directive removes Compact-v2's separate secret-shaped screen too.
  ...
  interactive controls; it has no separate page-`text` field or read-path
  redactor. Canonical tree shape and refs are preserved. Out of scope and
  unchanged: the vault's write-only property and `use_credential`'s server-side
  injection, the payment approval flow / 3DS / human approval, and the
  closed-vocabulary screen on the stderr audit trail and the registry-bound
  recipe action trace.
...
- **GitHub and npm now share one authored README.** The npm pack lifecycle
  copies the repository README into the tarball byte-for-byte and removes the
  generated package-local copy afterward; a smoke test prevents future drift.
- **Discovery metadata now uses the same plain-language description.** npm
  keywords, product homepage, MCP Registry copy, and Goose configuration no
  longer lead with internal product terminology.
...
- release(mcp): 1.0.31
- fix(mcp): issueMachineToken times out instead of hanging connect forever
- chore(release): resync staging with main (no-op pins)
- release(mcp): 1.0.30
- fix(mcp): interactive connect no longer discards fresh inbox-OTP consent
- chore(release): resync staging with main (no-op pins)
- release(mcp): 1.0.29
- fix(mcp): operate_store_login covers the signin_url host (Plunk browser-fill 403)
- chore(release): resync staging with main (no-op pins)
- chore(registry): entry-url backfill script (strip per-account signup_url segments)
- release(mcp): 1.0.28
- fix(mcp): pick the NEWEST verification link in a multi-message thread
- chore(release): resync staging with main (no-op pins)
- release(mcp): 1.0.27
