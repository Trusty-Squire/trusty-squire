...
it("keeps a draining recovery endpoint reachable while refusing mutations", async () => {
  ...
  try {
    ...
    expect(
      await brokerShutdownCleanupComplete({ active: 0, quarantined: 1, admitting: 0 }, async () => {
...
itWithChromium(
  "keeps the real daemon endpoint recoverable when real browser cleanup remains unproven",
  async () => {
              ...
              await Promise.all(
                entries
                  ...
                  .map(async (entry) => {
                    ...
                    return manifest.resources?.find(
                      (resource) => resource.user_data_dir === profile,
                    )?.pid;
                  }),
              ),
          )
