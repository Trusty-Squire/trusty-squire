...
describe("3-D Secure resolution", () => {
  ...
  it.skipIf(!chromiumAvailable)(
    "includes a decline appearing during click actionability wait in the dispatch baseline",
    async () => {
      const browser = await chromium.launch({ headless: true });
      try {
        const page = await browser.newPage();
        await page.setContent(`
          ...
          <script>
            ...
            Object.defineProperty(pay, "__tsPaymentSubmitDispatchListener", {
              configurable: true,
              get: () => dispatchListener,
              set: (value) => {
                dispatchListener = value;
                if (typeof value !== "function") return;
                pay.disabled = true;
                setTimeout(() => {
                  document.body.insertAdjacentHTML("beforeend", "<p>Payment declined</p>");
                }, 100);
                setTimeout(() => {
                  pay.disabled = false;
                }, 600);
            ...
            pay.addEventListener("click", () => {
              const frame = document.createElement("iframe");
              frame.title = "3D Secure authentication";
              frame.srcdoc = "<div>Approve in your bank app</div>";
              document.body.append(frame);
            });
          </script>
        `);
