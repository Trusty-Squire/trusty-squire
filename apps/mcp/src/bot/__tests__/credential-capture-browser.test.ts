...
it.each(["[role=dialog] input", "[role=dialog] section input", '[data-caption="a > b, c"] input'])(
  "walks cross-shadow descendants for %s when the CSS engine misses",
  async (selector) => {
    await page.setContent('<div role="dialog" data-caption="a > b, c" id="host"></div>');
    await page.locator("#host").evaluate((host) => {
      const shadow = host.attachShadow({ mode: "open" });
      shadow.innerHTML =
        '<section><label>Created key</label><input value="shadow-fixture"></section>';
    });
    ...
    try {
      ...
      expect(
        await captureCredentialSource("fixture", { selector: "[role=region] input" }),
...
it("preserves diagnostics when a Playwright-only selector matches nothing", async () => {
  ...
  expect(
    await captureCredentialSource("fixture", { selector: 'label:text-is("Missing")' }),
...
it("counts shadow candidates even when the selector engine returns one light-DOM match", async () => {
  await page.setContent(
    '<section role="dialog"><input id="light" value="light-fixture"><div id="host"></div></section>',
  );
  await page.locator("#host").evaluate((host) => {
  ...
  const lookup = vi.spyOn(page, "locator").mockReturnValue(locator);
  try {
    expect(await captureCredentialSource("fixture", { selector: "[role=dialog] input" })).toEqual({
      candidate_count: 2,
    });
...
it.each(["for", "aria-labelledby"])(
  "resolves %s names within the source's owning root",
  async (association) => {
    ...
    try {
      ...
      expect(
        await captureCredentialSource("fixture", { role: "textbox", name: "New key" }),
...
it.each([0, 1, 2])("counts %s sources across two matching containers", async (count) => {
  await page.setContent(
    '<section role="dialog" aria-label="First">First</section><section role="dialog" aria-label="Second">Second</section>',
  );
  await page.locator("section").evaluateAll((sections, inputCount) => {
    for (let i = 0; i < inputCount; i++) sections[i]!.innerHTML = '<input value="key-fixture">';
  }, count);
  const locator = page.getByRole("dialog");
  vi.spyOn(locator, "elementHandles").mockResolvedValue([]);
  vi.spyOn(locator, "getByRole").mockReturnValue(locator);
...
it.each([false, true])(
  "preserves normalized container matches with shadow competitor %s",
  async (competing) => {
    await page.setContent(
      '<section role="dialog" aria-labelledby="heading"><h2 id="heading">Created\n key</h2><input id="light" value="new-fixture"><div id="host"></div></section>',
    );
    if (competing) {
      await page.locator("#host").evaluate((host) => {
...
it.each(["element", "ancestor", "shadow-host"])(
  "excludes a background textbox hidden by its %s",
  async (kind) => {
    await page.setContent(
      '<div id="background"></div><section role="dialog"><input value="new-fixture"></section>',
    );
    await page.locator("#background").evaluate((background, hiddenBy) => {
      if (hiddenBy === "shadow-host") {
        ...
        (hiddenBy === "element" ? background.firstElementChild! : background).setAttribute(
          "aria-hidden",
          "true",
        );
      }
...
it.each(["textbox", "selector"])(
  "excludes an ARIA-hidden container for %s capture",
  async (kind) => {
    await page.setContent(
      '<div aria-hidden="true"><section role="dialog"><input value="hidden-fixture"></section></div>',
    );
    const result = await captureCredentialSource("fixture", {
...
it.each([false, true])(
  "keeps datalist inputs out of textbox captures with key present %s",
  async (withKey) => {
    await page.setContent('<div id="host"></div>');
...
it.each([false, true])(
  "preserves scoped child selectors with direct input present %s",
  async (direct) => {
    await page.setContent(
      '<section role="dialog"><section><input value="nested-old-fixture"></section>' +
        (direct ? '<input value="direct-new-fixture">' : "") +
        "</section>",
    );
    const result = await captureCredentialSource("fixture", {
...
it.each(["light", "shadow"])(
  "never follows a selector chain above its dialog into %s DOM",
  async (kind) => {
    await page.setContent(
      '<div class="created-key"><section role="dialog"><div id="host"></div></section></div>',
    );
...
it.each([
  ...
  "limits fallback chains to their container and shadow boundary: $tree $selector",
  async ({ tree, selector, count }) => {
    await page.setContent('<section role="dialog"><div id="host"></div></section>');
    await page.locator("#host").evaluate((host, kind) => {
      const root = kind === "shadow" ? host.attachShadow({ mode: "open" }) : host;
      root.innerHTML =
        '<section class="created-key"><input value="shadow-chain-fixture"></section>';
    }, tree);
...
it("returns operate_extract capture receipts from a real open-shadow dialog", async () => {
  ...
  const store = vi.fn(async (input: unknown) => {
    writes.push(input);
    return {
      reference: "vault://fixture/groq/created-key",
      service: "Groq fixture",
      label: "created",
      field_names: ["value"],
      allowed_hosts: ["groq.example.test"],
      updated: false,
    };
  });
  const api = { storeCredential: store } as unknown as ApiClient;
  const receipts: unknown[] = [];
  const evidence = process.env.CAPTURE_TEST_EVIDENCE_DIR;
  try {
    await page.setContent(
      '<h1>Groq-style capture fixture</h1><p>Synthetic key; local browser fixture, not the live Groq website.</p><section role="dialog" aria-label="Created key"><h2>Created key</h2><div id="host"></div></section>',
    );
    await page.locator("#host").evaluate((host, key) => {
      host.attachShadow({ mode: "open" }).innerHTML =
        '<input value="' + key + '" style="width:360px">';
    }, value);
    if (evidence) {
        ...
        () =>
          provisionExtractTool.handler(
            provisionExtractTool.inputSchema.parse({
              session_id: "fixture",
              capture: { store: { service: "Groq fixture", label: "created" }, source },
            }),
            api,
          ),
        undefined,
        { operationId: "fixture-extract-" + receipts.length, onCapture: async () => {} },
      );
    ...
    for (const source of [
      ...
      expect(await invoke(source)).toMatchObject({
        stored: true,
        stored_credential: { reference: "vault://fixture/groq/created-key" },
      });
    }
    ...
    await page.setContent(
      '<section role="dialog">First</section><section role="dialog">Second</section>',
    );
    ...
    if (evidence)
      await writeFile(
        join(evidence, "operate-extract-responses.json"),
        JSON.stringify(
          {
            environment:
              "Real headless Chromium, real capture resolver and operate_extract handler; session lookup and vault API stubbed; synthetic values only.",
            receipts,
            storage: {
              writes: writes.length,
              exactSyntheticValueReceived: true,
              writesOnUnresolvedOrAmbiguous: 0,
            },
          },
          null,
          2,
