...
import { provisionObserveTool, operateFillCredentialTool, operateTypeTool, operateClickTool } from "../../tools/provision-drive.js";
...
it("reads current login controls repeatedly and fills the matching saved fields in Chromium", async () => {
  ...
  try {
    ...
    const start = await startHarnessProvisionSession({
      browser: BrowserController.fromHarnessPage(page), serviceUrl: "https://fixture.test/login",
    ...
    let guidance = "";
    try { await operateTypeTool.handler(args, null); } catch (error) { guidance = String(error); }
    transcript.push({ tool: "operate_type", arguments: args, error: guidance });
    expect(guidance).toMatch(/operate_fill_credential.*list_credentials.*field_names.*operate_type/);
    expect(await page.locator("#password").inputValue()).toBe("");
    const api = {
      browserFillCredential: async (input: { fields: string[]; current_host: string; encrypted_response_public_key: string }) => {
        expect(input.current_host).toBe("https://fixture.test/login");
        expect(input.fields).toEqual(["username", "password"]);
        const encrypt = (value: string) => publicEncrypt({ key: input.encrypted_response_public_key,
          padding: constants.RSA_PKCS1_OAEP_PADDING, oaepHash: "sha256" }, Buffer.from(value)).toString("base64");
        return { reference: "vault://synthetic/login", encrypted_fields: {
          username: encrypt("ada@example.test"), password: encrypt("synthetic-password"),
        } };
      },
    } as unknown as ApiClient;
    const loadArgs = operateFillCredentialTool.inputSchema.parse({ session_id: sessionId,
      reference: "vault://synthetic/login", fields: ["username", "password"] });
    const loaded = await operateFillCredentialTool.handler(loadArgs, api) as { slots: Record<string, { slot: string }> };
    transcript.push({ tool: "operate_fill_credential (synthetic encrypted vault response)", arguments: loadArgs, response: loaded });
    expect(JSON.stringify(loaded)).not.toContain("synthetic-password");
    for (const [row, field] of [[email, "username"], [password, "password"]] as const) {
      const input = { session_id: sessionId, ref: row[0]!, slot: loaded.slots[field]!.slot };
      transcript.push({ tool: "operate_type", arguments: input, response: await operateTypeTool.handler(input, null) });
    ...
    if (evidence) {
      ...
      await writeFile(join(evidence, "operator-login-responses.json"), JSON.stringify(transcript, null, 2));
    }
  } finally {
