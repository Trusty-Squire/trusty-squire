...
it("reconciles benign unsettled dispatched work in-band and admits a fresh start without reconnect", async () => {
  const root = await mkdtemp(join(tmpdir(), "ts-forward-benign-"));
  const path = join(root, "b.sock");
  ...
  // The click is dispatched and then its reply is lost forever (cancelled
  // benign browsing against a broker that never answers the cancellation).
  let neverReply: ((reply: unknown) => void) | undefined;
  const broker = await listenBroker(path, {
    authenticate: async () => {
      authentications += 1;
    ...
    call: async (_principal, method, params) => {
      ...
      if (params.name === "operate_start") {
        ...
        return { capability, result: { session_id: sessionId } };
      }
      if (params.name === "operate_click")
        await new Promise(() => {
          neverReply = undefined; // never settles; reply lost with the broker
        });
      throw new Error(`Unexpected ${method}`);
    },
    disconnect: async () => undefined,
  ...
  try {
    ...
    void neverReply;
    // The cancelled click's invoke never settles; a fresh start on the SAME
    // lineage must still be admitted in-band (single connection, no
    // reconnect) after the bounded reconciliation grace.
    const second = (await forwarder.invoke("operate_start", {}, "start-2")) as {
      session_id: string;
    };
