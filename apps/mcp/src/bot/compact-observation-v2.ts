...
export function safePageSemanticsV2(source: ObservationSemanticSourceV2): SafePageSemanticsV2 {
  ...
  const headings = source.headings
    ...
    .slice(0, 1);
  const errorPage = errorPageBlockerV2(title, headings);
  return {
 ...
 * code-owned title/heading source. Signatures are deliberately narrow — vendor
 * attribution phrases and exact error-page titles — so ordinary content that
 * merely discusses HTTP errors never matches.
 */
...
export function errorPageBlockerV2(
  ...
  const candidates: Array<{ text: string; matched: string }> = [];
  const exactTitle = normalized(title);
  if (exactTitle !== undefined && ERROR_PAGE_EXACT_TITLES.has(exactTitle.toLowerCase()))
    candidates.push({ text: exactTitle, matched: exactTitle });
  for (const source of [title, ...headings]) {
    const value = normalized(source);
    if (value === undefined) continue;
    for (const signal of ERROR_PAGE_SIGNAL_RES) {
      if (signal.test(value)) {
        candidates.push({ text: value, matched: value });
        break;
      }
    }
  }
  ...
  return first === undefined
    ? undefined
    : { kind: "error_page", text: boundedBlockerTextV2(first.text) ?? first.text };
}
...
export function safeBlockersV2(
  ...
  for (const node of nodes) {
    ...
    const detail = boundedBlockerTextV2(browserUseBoundedContextText(node, DIALOG_DETAIL_MAX_CHARS));
    const options = controls
      .slice(0, DIALOG_MAX_OPTIONS)
      .map((control) => {
        const label = boundedBlockerTextV2(blockerTextV2(control));
