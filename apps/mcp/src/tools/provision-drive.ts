...
const CONTROL_QUERY_CONTRACT =
  ...
  "A dialog blocker also carries options — a bounded list of its rendered controls in DOM order, including the close path that keeps what was entered whenever the dialog renders one; when more controls exist than fit, that dismiss path and the controls that resolve the dialog are kept ahead of anchors, so query the control map for the rest. ref names a dismissal control or nothing: a dialog whose controls are only choices, links, or unlabelled-as-dismiss buttons (an address dialog offering Use suggested / Keep what I entered) reports those in options with target=unavailable rather than advertising one of them as the way out. A bounded detail carries the dialog's own prose, absent when it would only repeat text. " +
