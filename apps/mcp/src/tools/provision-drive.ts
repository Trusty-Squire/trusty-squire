...
const CONTROL_QUERY_CONTRACT =
  ...
  "A dialog blocker also carries options — a bounded list of its rendered controls in DOM order, including the close path that keeps what was entered whenever the dialog renders one; when more controls exist than fit, that dismiss path and the controls that resolve the dialog are kept ahead of anchors, so query the control map for the rest. ref names an exit or nothing: a button whose WHOLE label is a close/cancel form or a keep-what-was-entered form, preferring the latter. A label that merely contains an exit word is not one — Cancel subscription and Close account are the destructive confirm, so a dialog offering only those reports them in options with target=unavailable rather than advertising one as the way out, and the same holds for a dialog whose controls are only choices or links. A bounded detail carries the dialog's own prose, absent when the dialog had no name of its own and text is already that prose. " +
