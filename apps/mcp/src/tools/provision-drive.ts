...
const CONTROL_QUERY_CONTRACT =
  ...
  "A dialog blocker also carries options — a bounded list of its rendered controls in DOM order, including the close path that keeps what was entered whenever the dialog renders one; when more controls exist than fit, that dismiss path and the controls that resolve the dialog are kept ahead of anchors, so query the control map for the rest. A dialog offering only choices or links reports exactly those, with target=unavailable. A bounded detail carries the dialog's own prose (not its name or its control labels, which the blocker already reports). " +
