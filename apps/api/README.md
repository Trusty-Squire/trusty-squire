...
| `GET` | `/v1/vault/mutation-approvals/:id/ceremony` | web | Show the capability-bound mutation intent for phone approval |
...
`wait_ms` optionally shortens one `wait_for_submission=1` request to 0-15000
milliseconds; each wait re-reads the approval status and returns promptly on
...
agent sessions may create and poll their own account's approvals; the owner web
...
the approval terminal. Any failure rolls the transaction back. Approval rows,
Telegram prompts, and responses contain only labels and host allowlists—never
ciphertext or secret field values.
