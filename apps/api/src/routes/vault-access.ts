...
export const registerVaultAccessRoute: FastifyPluginAsync<{
  ...
  // use_credential buffers the response and hands it back to the AGENT (into
  // its context), which is what this cap is for — the egress path streams and
  // carries no size cap at all. The old 10KB default was too small for real API
  // responses and web pages — an agent GETting a Shopify HTML page tripped it.
  // Default 2MB; tune via VAULT_USE_MAX_RESPONSE_BYTES without a redeploy. Guard
  // a non-numeric / <= 0 env so a bad value can't disable the cap.
