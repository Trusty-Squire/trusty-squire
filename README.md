...
`connect` signs you in with Google or GitHub, detects your coding agent, and merges the `squire` MCP server into its existing configuration. Sign-in runs in Trusty Squire's own shared browser, which lives on a private display the daemon owns — so on every platform the sign-in is shown through a single-use noVNC URL printed during the ceremony, never through a Chrome window popped on your desktop (there is no local sign-in window on macOS or Windows today). On a headless Linux server, including an SSH/TTY session with an inherited virtual display, the ceremony starts the login-scoped Xvfb + noVNC stack on demand and prints that URL, which shows the whole shared browser display for as long as the ceremony runs and stops working when it ends. The default quick tunnel and every local helper are torn down when that ceremony completes, times out, fails, or is interrupted. Operators may instead set both `TS_LOGIN_PUBLIC_HOSTNAME` and `TS_LOGIN_LOCAL_PORT` to reuse an externally managed named tunnel; Trusty Squire still tears down its per-ceremony display and local listener, but never creates or stops that external tunnel. If that tunnel's fixed local port is busy, login reports it and uses a one-off quick tunnel for that sign-in instead. Restart the agent and ask for the finished website outcome. Trusty Squire is free to start.
