...
- `state` — one of four:
  ...
  - `busy` — Squire could not read or use the bot profile to answer, including a ceremony the profile gate refused because another browser holds it. `holder` says who has it.
...
- `holder` — who holds the bot profile, as read: `{kind:"none"}`, `{kind:"other",code:"singleton_lock",pid}`, or `{kind:"unknown",reason:"cross_host"}`. A lock left behind by a dead process is `none`, not a holder, and `busy` with `holder:{kind:"none"}` is an honest answer — nothing holds the profile, Squire just could not verify it (see `reason`).
- `browser_location` — where the ceremony browser actually opened, reported by the code that placed it: `{kind:"host_screen",display?}` (the screen you are at), `{kind:"virtual"}` (a display nobody is sitting at — its noVNC address is live only while the ceremony runs and is printed on stderr then; the tunnel and the browser it showed are both gone by the time this report exists, so no address is carried here), `{kind:"unreachable",reason}`, `{kind:"none"}` (no browser was opened), or `{kind:"unknown",reason}` (a browser opened that Squire did not place — `--skip-browser` hands the link to the machine's default browser — or the ceremony ended before any path reported). Squire decides this; callers do not detect screens.
