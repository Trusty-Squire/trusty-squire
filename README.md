...
- `account` — `{ id, providers }` whenever the run proved which account this machine is bound to, which is not only when it ends `connected`: a `no-browser` run whose provider session or scoped refresh missed, and a `busy` run whose stored session names an account, both carry it. `providers` lists only the provider sessions actually observed, so `[]` means none were seen — not that the binding is absent. `null` when the run never proved a binding.
