...
Every exit path answers, including a run that fails before it resolves a target, so the machine channel is never empty. The object is a connect report (it carries `state`) unless connect refused the arguments, in which case it is `{"error":"usage","message":"…"}` and the exit code is 64 — a rejected flag is not a connection state.
...
- `holder` — who holds the bot profile, as read: `{kind:"none"}`, `{kind:"other",code:"singleton_lock"|"operation_lease",pid}`, or `{kind:"unknown",reason:"cross_host"}`. `singleton_lock` is a live Chrome on the profile; `operation_lease` is another Squire run that claimed it for work (a `--force-relogin` wipe holds the lease with no browser running). A lock left behind by a dead process is `none`, not a holder, and `busy` with `holder:{kind:"none"}` means nothing holds the profile — Squire just could not verify it (see `reason`).
- `browser_location` — where the ceremony browser actually opened, reported by the code that placed it: `{kind:"host_screen",display?}` (the screen you are at), `{kind:"virtual"}` (a display nobody is sitting at — reported only when the run claimed, because its noVNC address is live only while the ceremony runs and is printed on stderr then; a run that did NOT claim reports `unreachable` instead, since the tunnel that reached the display is gone by the time this report exists and no dead address is ever carried here), `{kind:"unreachable",reason}`, `{kind:"none"}` (no browser was opened), or `{kind:"unknown",reason}` (a browser opened that Squire did not place — `--skip-browser` hands the link to the machine's default browser — or the ceremony ended before any path reported). Squire decides this; callers do not detect screens.
