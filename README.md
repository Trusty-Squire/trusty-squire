...
- `state` — one of four:
  ...
  - `no-browser` — no browser here is signed in the way this install needs: the sign-in happened outside Squire's browser, the ceremony finished without the session the operator needs, or the link lapsed — `reason` names those. The remaining cause, "nothing here could be shown the page", carries `reason: null` and is named by `browser_location: {kind:"unreachable"}` instead.
