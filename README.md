...
- `state` — one of four:
  ...
  - `busy` — Squire could not read or use the bot profile to answer, including a ceremony refused because another session holds the browser (the profile gate's refusal, or a resident broker's). `holder` says who has it.
...
- `account` — `{ id, providers }` whenever the run proved which account this machine is bound to, which includes a `no-browser` run whose scoped provider refresh missed. `null` when it did not.
