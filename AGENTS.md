...
### 11. A merchant's own pre-selected saved/stored card can win over the vaulted card operate_pay filled — and the post-submit 3DS wait had no resume path

Root-caused in `ts-operator-payment-defects` from a live Hibiya Kadan (EbisuMart) repro: `operate_pay` released and audited the exact card identified by `card_ref` (the crypto-bound AAD check in `pay-operator.ts`'s `executeOperatePay` — mismatched candidates fail closed with `payload_hash_mismatch`, so a wrong-card *release* is not reachable through that path), yet the merchant's 3-D Secure authenticated through a different card's issuer. The server-side vault lookup (`getByIdForAccount` in `apps/api`, a direct by-ID+by-account fetch) and the `/ceremony` endpoint were also traced clean. The actual gap: a repeat-customer checkout can render a "new card" fieldset (what we fill) alongside an already-**checked** "use my saved card" radio for a *different* stored card — filling the new-card fields never unchecks that radio, so the checkout can submit the merchant's own default instead.

**The rule:** `submitFilledCheckoutInScope` (`browser.ts`) runs `detectCompetingSavedCardSelection` across every page frame and recursively through every open shadow root before it clicks a pay/place-order control. Checked native/ARIA radios and selected `<option>`s are candidates; their own text, ARIA label, wrapping label, and `label[for]` association are checked for masked-PAN or explicit saved-card markers (`•••• 1234`, "ending in 1234", "card on file", "saved card", JP equivalents). Only the exact card fields stamped `data-ts-sealed-payment` during the approved fill are excluded — never an entire form or frame. Any traversal or inspection error fails closed with `payment_card_selection_ambiguous`. See `"refuses a saved-card selection nested in open shadow roots"`, `"refuses a selected saved-card option alongside the filled card fields"`, and the paired false-positive guard test in `browser-payment.test.ts`.

**Companion gap, same investigation:** the post-submit 3DS wait (`waitForThreeDsResolution`, unchanged since the rc.21→rc.23 "native completion" decision — see `ThreeDsResolution`'s doc comment) is a single bounded call (≤600s). When it timed out, `operate_pay` handed back a terminal-shaped `payment_3ds_required`/`payment_outcome_unknown` and **retained zero session state** — unlike the pre-charge approval wait, which has had `operate_payment_status` + `PendingApprovalWait` since `#486`. A decoupled/out-of-band (app-push) challenge's real completion time (cardholder notices, unlocks phone, opens the banking app, approves) routinely exceeds one bounded wait, and nothing was left watching. Fixed additively: `Session.pendingThreeDs` (deliberately NOT part of the `activePayment` lease union — the card is already released and the charge already submitted, so there is no re-authorization risk) is set whenever `operate_pay` hands back an unresolved 3DS status — including a `three_ds_wait_seconds:0` call, since skipping the wait skips polling, not tracking. While it's set, `claimActivePaymentForOperatePay` refuses a new `operate_pay` lease and `enforcePlaceOrderGuard` refuses an `operate_act` charge-click, both pointing the host at `operate_payment_status`, which re-polls the SAME live browser via `threeDsStatusResult` (a compare-and-clear on the exact pending-wait instance, so a race between two status calls can't drop a fresher pending state), bounded by a 20-minute resumable window (`THREE_DS_RESUME_WINDOW_MS`) before handing back an accurate `payment_3ds_unresolved`. `pendingThreeDs` is cleared only on a terminal status, the deadline, or session close — `closeFinishingProvisionSession`/`closeAllProvisionSessions` both do one live `waitForThreeDsResolution(0)` check and write an audit record before clearing it, so an abandoned charge is never dropped silently. **Known residual gaps** (money-fence still holds — refusal, not silent wrongdoing — but coverage isn't total): the saved-card fence re-checks once at the top of `submitFilledCheckoutInScope`, not again immediately before the click (a TOCTOU window if the checkout mutates its selection in between); and a charge-click via `operate_act` during `operate_pay`'s OWN in-progress 3DS wait (before `pendingThreeDs` is set) isn't yet blocked by `enforcePlaceOrderGuard`, only by the pre-existing `activePayment: "operating"` lease blocking a second `operate_pay` call. See `pay-operator.test.ts`'s `"a timed-out 3DS wait persists resumable state"` and `operate-session-flow.test.ts`'s `"operate_payment_status — resumable post-submit 3DS wait"` block.
