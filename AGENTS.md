...
### 9. Stable cuts repair workspace pins and sync release ancestry back to `staging`
...
Separately: `1.1.9`/`1.1.10` squash-merged their release PRs into `main`, which severed `main`'s ancestry from `staging` and made the next release PR conflict (forcing a manual `-X ours` reconciliation on `1.1.10`); PR-triggered CI also attached zero times while that PR was conflicted. Stable PRs must use **"Create a merge commit," never squash**. A merge commit alone is not enough, because its release-only version and changelog commit exists only on `main`. After a verified stable publish, `.github/workflows/release.yml` runs `tools/sync-release-ancestry.mjs` to create the equivalent of `git merge -s ours main` on `staging`: the stable commit becomes an ancestor while staging's prerelease tree stays byte-identical. Keep that sync step after canonical publish verification and GitHub release creation.
