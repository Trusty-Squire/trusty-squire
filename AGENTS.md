...
**The rule:** `fillCheckoutCardIntoFrames`/`submitFilledCheckoutInScope` (`browser.ts`) now refuse to click the pay/place-order control — `detectCompetingSavedCardSelection`, throwing `payment_card_selection_ambiguous` — when a checked radio outside the filled card group's own form shows a masked-PAN or explicit saved-card marker (`•••• 1234`, "ending in 1234", "card on file", "saved card", JP equivalents). Scoped to radios only, never `<select>`, because when the card group's root resolves to the whole `<form>` (no nested fieldset), *every* form control — including our own expiry `<select>`s — gets stamped as part of the group, so containment can't distinguish "ours" from "theirs" for selects; a checked radio is never one of ours. See `"refuses to submit when an already-selected saved card would win over the filled card"` and the paired false-positive guard test in `browser-payment.test.ts`.
