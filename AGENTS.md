...
- `BrowserController` local launches stay headed. A real screen wins:
  `hasDisplay()` (`apps/mcp/src/bot/display-env.ts`) means use the machine
  display; Xvfb and the noVNC rig exist only on headless hosts.
  `ownedHeadedBrowserEnvironment` in
  `apps/mcp/src/bot/browser-process-owner.ts` is the launch-time gate.
  Do not start Xvfb when the host already has a screen.
